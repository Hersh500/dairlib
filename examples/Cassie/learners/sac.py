...
if not to_eval:
    try:
        ...
        if model_fname is not None:
            ...
            model = SAC("MultiInputPolicy", env, verbose = rl_params["verbose"], tensorboard_log = logdir, policy_kwargs=dict(normalize_images=False), learning_starts = rl_params["learning_starts"], batch_size = rl_params["batch_size"], buffer_size = rl_params["buffer_size"])
        model.learn(total_timesteps = rl_params['num_steps'])
        model.save(logdir + "sac-cassie_joy")
