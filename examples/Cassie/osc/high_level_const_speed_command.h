...
/// `HighLevelConstSpeedCommand` calculates desired velocity (~in local frame)
/// and desired horizontal velocity (in local frame) of Cassie's pelvis.
///
...
class HighLevelConstSpeedCommand : public drake::systems::LeafSystem<double> {
 public:
  ...
  HighLevelConstSpeedCommand(
      ...
      const Eigen::Vector2d& desired_vel, double ramp_time,
      int footstep_option);
