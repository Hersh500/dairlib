...
//                                       drake::multibody::CoulombFriction(0.8, 0.8));
    ...
    void generateDitchesObstacleCourse(MultibodyPlant<double> *plant, double dropout_pct) {
      ...
      double pixel_size_m = 0.5;
      double height = 0.5;
      int name_count = 0;
      for (double x = -8; x <  8; x = x + pixel_size_m) {
        for (double y = -8; y < 8; y = y + pixel_size_m) {
          // Make sure the robot doesn't spawn on top of a hole
          if (!drop(rng) || (x < 1 && x > -1 && y < 1 && y > -1)) {
              // std::cout << "adding ground at" << x << ", " << y << std::endl;
              RigidTransform<double> pose = RigidTransform<double>(drake::math::RollPitchYaw<double>(0, 0, 0),
                                                                   Eigen::Vector3d(x, y, -0.05));

              plant->RegisterCollisionGeometry(plant->world_body(), pose, drake::geometry::Box(pixel_size_m+0.05, pixel_size_m+0.05, 0.1),
                                               "box_collision_"+ std::to_string(name_count), drake::multibody::CoulombFriction(0.8, 0.8));

              plant->RegisterVisualGeometry(plant->world_body(), pose, drake::geometry::Box(pixel_size_m+0.05, pixel_size_m+0.05, 0.1),
